...
class UsuarioCrudAdminViewController: UIViewController {
    ...
    @IBAction func btnGrabarTapped(_ sender: UIButton) {
        guard let id = usuarioRecibido?.id,
              ...
              let clave = tfClave.text, !clave.isEmpty else { return }
        
        let usuarioEditado = Usuario(id: id, nombre: nombre, email: email, clave: clave, rol: usuarioRecibido?.rol ?? "usuario")
    ...
    @IBAction func btnEliminarTapped(_ sender: UIButton) {
        ...
        let alerta = UIAlertController(title: "Eliminar", message: "¿Seguro?", preferredStyle: .alert)
