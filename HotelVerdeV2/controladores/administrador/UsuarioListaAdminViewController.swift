...
class UsuarioListaAdminViewController: UIViewController, UITableViewDelegate,
    UITableViewDataSource
{
    ...
    func tableView(
        ...
        performSegue(
            withIdentifier: "verDetalleUsuario", sender: usuarioSeleccionado)
        tableView.deselectRow(at: indexPath, animated: true)
    }

    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "verDetalleUsuario" {
            if let destino = segue.destination
                as? UsuarioCrudAdminViewController
            {
